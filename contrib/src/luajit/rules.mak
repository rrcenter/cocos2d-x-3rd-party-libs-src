...
LUAJIT_GITURL := https://github.com/LuaJIT/LuaJIT
...
ifdef HAVE_MACOSX
	cd $< && CFLAGS="-DLUAJIT_ENABLE_GC64" LD_FLAGS="" $(MAKE) -j8 MACOSX_DEPLOYMENT_TARGET=10.14
endif
...
ifeq ($(MY_TARGET_ARCH),x86_64)
	cd $< && $(HOSTVARS_PIC) $(MAKE) -j8 HOST_CC="$(CC)" HOST_CFLAGS="$(CFLAGS)" MACOSX_DEPLOYMENT_TARGET=10.14
else
...
ifeq ($(MY_TARGET_ARCH),x86_64)
	...
	cd $< &&  $(MAKE) -j8 HOST_CC=$(LUAJIT_HOST_CC) CROSS=$(LUAJIT_CROSS_HOST) TARGET_SYS=iOS  TARGET_FLAGS=$(LUAJIT_TARGET_FLAGS) MACOSX_DEPLOYMENT_TARGET=10.14
endif
